...
def rearrange_compound_guides(apps, schema_editor):
    ...
    dups = Compound.objects.exclude(CSID=None).values('CSID').annotate(csid_count=Count('id')).order_by().filter(csid_count__gt=1)
    for compound in Compound.objects.all():
        if compound.CSID is not None: 
            actualCompound = Compound.objects.filter(CSID=compound.CSID)[0] # Ignore duplicate compounds
        else:
            acutalCompound = compound # We don't want to accidentally strip out all of the custom compounds.
        if not CompoundGuideEntry.objects.filter(compound=actualCompound).exists():
            CompoundGuideEntry.objects.create(compound=actualCompound, labGroup=compound.labGroup, abbrev=compound.abbrev)
