...
class MultiQuerySet(object):
    ...
    def order_by(self, *args):
        ...
        raise NotImplementedError(
            "No order_by implemented for MultiQuerySet. File a feature request if you need this feature.")
...
class ArffQuerySet(models.query.QuerySet):
    ...
    def arffHeaders(self, whitelist=None):
        ...
        the csv file query set. This will manage foreignkey relations (make sure to define __str__ on your models!),
    ...
    def toArff(self, writeable, expanded=False, relationName='relation', whitelistHeaders=None, missing="?"):
        ...
        writeable.write(
            b'%arff file generated by the Dark Reactions Project provided by Haverford College\n')
        writeable.write('\n@relation {}\n'.format(relationName).encode())
        if expanded:
        ...
        for row in self.rows(expanded, whitelistHeaders):
            writeable.write(','.join(('"' + str(row.get(key)) + '"' if (row.get(key)
                                                                        is not None) else missing) for key in headers.keys()).encode())
            writeable.write(b'\n')
    ...
    def rows(self, expanded=False):
        """Return a dictionary for writing a row of data using DictWriter from csv module."""
        for item in self:
            yield {field.name: getattr(item, field.name) for field in self.model._meta.fields}
