        ...
        if component == 'performed_reactions':
            ...
            if component == 'reactions':
                ...
                labGroups = LabGroup.objects.filter(
                    reaction__in=[p for p in performedReactions]).distinct()
                if component == 'lab_groups':
                    ...
                    if component == 'users':
                        ...
                        licenseAgreements = LicenseAgreement.objects.filter(
                            user__in=users).distinct()
                        if component == 'license_agreements':
                            ...
                            licenses = License.objects.filter(
                                licenseagreement__in=licenseAgreements).distinct()
                            if component == 'licenses':
                                return HttpResponse(serializers.serialize('xml', licenses), content_type='Application/xml')
                            else:
                                compoundQuantities = CompoundQuantity.objects.filter(
                                    reaction__in=[p for p in performedReactions]).distinct()
                                if component == 'compound_quantities':
                                    ...
                                    compoundRoles = CompoundRole.objects.filter(
                                        compoundquantity__in=compoundQuantities).distinct()
                                    if component == 'compound_roles':
                                        ...
                                        if component == 'compounds':
                                            ...
                                            chemicalClasses = ChemicalClass.objects.filter(
                                                compound__in=compounds).distinct()
