...
for i, sm in enumerate(c.statsmodel_set.all()):
    ...
    fn = str(sm.inputFile)
    shutil.copyfile(fn, os.path.join(
        dest, str(i + 1) + '_train_' + fn.split('/')[-1]))
    descriptorHeaders = [d.csvHeader for d in chain(
        c.descriptors, c.outcomeDescriptors)]

    assert(sm.testSets.all().count() == 1)
    testSet = sm.testSets.all()[0]
    test_arff = mv._prepareArff(
        testSet.reactions.all(), descriptorHeaders, verbose=True)
    shutil.move(test_arff, os.path.join(
        dest, str(i + 1) + '_test_' + test_arff.split('/')[-1]))
