...
COMMENT="Legacy rxns. New descriptors without ChemAxon nonzero InfoGain. 15 mutual info split."
...
python -u -m cProfile -o "$FILE_SUFFIX.profile" $SCRIPT -p 'test' -rxn $RXN_SET_NAME -r $OUTCOME_DESCRIPTOR -s $SPLITTER -so "$SPLITTER_OPTIONS" -v -mt @"$MODEL_VISITOR_FILE" -vo @"$MODEL_VISITOR_OPTION_FILE" -d "$COMMENT"
