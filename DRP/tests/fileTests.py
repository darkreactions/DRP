...
pep8Files = [os.path.join(settings.BASE_DIR, os.path.join(*_pep8File)) for _pep8File in _pep8Files]
...
class TestFiles(unittest.TestCase):
  ...
  def test_settingsExamplePresent(self):
    ...
    self.assertTrue(os.path.isfile(settings_file), '{} is missing!'.format(settings_file))
  ...
  def testPep8(self):
    for fileName in pep8Files:
      ...
      self.assertEqual(result.total_errors, 0, 'Errors were found in pep8 Conformance:\n{}'.format(output))
