...
class CreateNew(DRPTestCase):
    ...
    def setUp(self):
        ...
        self.user1 = User(first_name='Aslan', password=make_password(
            'old_magic'), username="Aslan", email="aslan@example.com")
        self.user2 = User(first_name='White', last_name='Witch', password=make_password(
            'new_magic'), username="whitewitch", email="whitewitch@example.com")
        self.user1.save()
        self.user2.save()
...
class DuplicateUniqueValues(DRPTestCase):
    ...
    def test_duplicateTitle(self):
        ...
        form = LabGroupForm({'title': 'DuplicateValues', 'address': '2, war drobe, Narnia',
                             'email': 'whitewitch@example.com', 'accessCode': 'old_magic'})
        self.assertFalse(form.is_valid())
