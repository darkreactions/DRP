...
class CompoundFilterSetSucceed(BaseFormTest):
    ...
    def setUpFormData(self):
        """Three different forms searching for different compounds."""
        self.formData = {}
        ...
        self.formData[
            "form-0-INCHI"] = "InChI=1S/C7H16N2/c1-2-9-5-3-4-7(9)6-8/h7H,2-6,8H2,1H3"
        ...
        self.formData[
            "form-1-INCHI"] = "InChI=1S/C6H12N4/c1-7-2-9-4-8(1)5-10(3-7)6-9/h1-6H2"
        ...
        self.formData[
            "form-2-smiles"] = "[O-][V](=O)([O-])[O-].[Na+].[Na+].[Na+]"
    ...
    def setUp(self):
        ...
        self.setUpFormData()
        self.form = CompoundFilterFormSet(
            user=self.user, labGroup=self.labgroup, data=self.formData)
    ...
    def test_is_empty(self):
        """Test that this test is correctly evaluated as valid and not empty."""
        self.assertTrue(self.form.is_valid(), str(self.form.errors))
        self.assertFalse(self.form.is_empty())

    def test_cleaned_data(self):
        """Test that this will clean data correctly."""
        self.assertTrue(self.form.is_valid())
        cleaned = self.form.cleaned_data
        self.assertEqual(len(
            cleaned), 3, "Got more than one result from the filter form, or cleaned more than the results returned from the form")

    def test_fetch(self):
        """Test that the form fetch method works correctly."""
        self.assertTrue(self.form.is_valid())
        fetched = self.form.fetch()
        self.assertEqual(fetched.count(), 3,
                         "{} results found.".format(fetched.count()))
        self.assertEqual(fetched.get(abbrev="2-amep").abbrev, "2-amep",
                         "The result returned did not have the expected abbrev attr. Attribute given was {}".format(fetched[0].abbrev))
        self.assertEqual(fetched.get(abbrev='hmta').abbrev, "hmta",
                         "The result returned did not have the expected abbrev attr. Attribute given was {}".format(fetched[1].abbrev))
        self.assertEqual(fetched.get(abbrev="sov").abbrev, "sov",
                         "The result returned did not have the expected abbrev attr. Attribute given was {}".format(fetched[2].abbrev))
