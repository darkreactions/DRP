    ...
    def _single_split(self, reactions, key_counts, verbose=False):
        ...
        for key, count in key_counts:
            if (abs(test_size + count - goal_size) < abs(test_size - goal_size)) and (test_size + count < goal_size + margin):
                # Adding the next set gets us closer to the goal size without going over the error margin. Do it
                test_size += count
                test_keys.append(key)
            elif abs(test_size - goal_size) < margin:
                # We shouldn't add the next set and we're within the error margin. We're done
                break
            # The next set is too big and we're not within the error margin. Try skipping it
        else:
            raise RuntimeError('Failed to make a split under the given parameters.')
