"""A group custom formset classes. Used by filter forms but to be deprecated."""
...
class FormSetManagerForm(forms.Form):
    ...
    """This class acts as a management form for the FormSet class."""
    ...
    def clean(self):
        """
        Overridden clean method; check that the number of forms is correct.

        Alter the internal data to ensure that it matches the number of forms after any additions have been made.
        """
    ...
    def submittedForms(self):
        """Return the correct number of forms that were actually submitted with data on this occasion."""
    ...
    def as_ul(self):
        """
        Override the html output for this form since it only has submit buttons which don't have labels.
...
class FormSet(object):
    ...
    """A drop in replacement for django formsets which more accurately supports HTTP standard way of doing things where javascript cannot be depended upon."""
    ...
    def is_valid(self):
        """Check management form and all forms are valid and return false otherwise."""
        return self.managementForm.is_valid() and all(f.is_valid() for f in self.forms)

    @property
    def cleaned_data(self):
        """Return cleaned data for all valid forms."""
...
class ModelFormSet(FormSet):
    ...
    def save(self, commit=True):
        """Return the objects created by all valid forms in this formset."""
