...
def get_reactants_indices(reaction):
  ...
  for r_f, params in reactant_fields.items():
    ...
    if compound.compound not in CG: continue
    if units.lower() != "g": raise Exception("Units not 'g'!")
    if not mass: raise Exception("Mass was either 0 or non-numeric!")
...
def generate_grid(reaction, amine_list, debug=True):
  ...
  if model==None: raise Exception("Latest model could not be found!")
