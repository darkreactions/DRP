...
class CGEntry:
    def __init__(self, abbrev, fake_abbrev, smiles, m_type,jchem_path = "/home/praccugl/ChemAxon/JChem/bin", sdf_path="sdf", error_pipe = sys.stderr):
        ...
        atoms_count = {atom: atoms.count(atom) for atom in atoms} 
        self.properties_map = {"mw": self.calc_MW(), "NopH": self.calc_NopH(), "polsurf": self.calc_polSurf(), "msacc": self.calc_msacc(), "msdon": self.calc_msdon(), "projectionArea": self.calc_projectionArea(), "type": self.m_type, "atoms": list(set(atoms_from_smiles(smiles))), "abbrev": self.abbrev, "smiles": self.smiles, "atoms_count": atoms_count}
